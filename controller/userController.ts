...
const registerUser = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const restaurant_id_query = await pool.query('SELECT restaurant_id FROM users WHERE id = $1', [user_id]);
        const restaurant_id = restaurant_id_query.rows[0].restaurant_id;
